...
export function ChatWidget() {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        const response = await fetch(`${API_URL}/chat/${storedSessionId}`);
        if (response.ok) {
  ...
  const handleClearChat = async () => {
    if (!confirm('Bạn có chắc chắn muốn làm mới cuộc trò chuyện này?')) return;
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="mb-4 w-96 max-w-[calc(100vw-3rem)] h-[500px] max-h-[calc(100vh-8rem)] bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col overflow-hidden"
          >
            {/* Header */}
